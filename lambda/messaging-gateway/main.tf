terraform {
  ...
  # Local backend — terraform.tfstate is gitignored, same as other Lambda modules.
  backend "local" {}
}
...
# ── IAM ───────────────────────────────────────────────────────────────────────
# Least privilege: send to sync-events.fifo only, read the messaging bearer
# token from SSM (created manually — never in Terraform state), write logs.

resource "aws_iam_role" "lambda" {
  name = "messaging-gateway-lambda"
...
resource "aws_iam_role_policy" "lambda" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "MessagingTokenRead"
        Effect = "Allow"
        Action = ["ssm:GetParameter"]
        Resource = [
          "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter${var.messaging_token_param}",
        ]
      },
      {
        Sid      = "Logs"
...
resource "aws_lambda_function" "messaging_gateway" {
  function_name    = "messaging-gateway-receiver"
  ...
  environment {
    variables = {
      ...
      MESSAGING_TOKEN_PARAM = var.messaging_token_param
    }
  }
...
# Public HTTPS entry point for OpenClaw/WhatsApp deliveries. Auth NONE by
# design: request authenticity is enforced inside the handler via bearer token
# (ADR-014; daniel-ubuntu never exposes a port).
resource "aws_lambda_function_url" "messaging_gateway" {
  function_name      = aws_lambda_function.messaging_gateway.function_name
