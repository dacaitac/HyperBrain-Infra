...
data "aws_sqs_queue" "user_commands" {
...
# ── IAM ───────────────────────────────────────────────────────────────────────
# Least privilege: send to user-commands.fifo only, read the bearer token from
# SSM (SecureString — value never in Terraform state), write CloudWatch logs.

resource "aws_iam_role" "lambda" {
  name = "replan-agenda-lambda"
    ...
    Statement = [
      {
        ...
        Resource = [data.aws_sqs_queue.user_commands.arn]
      },
      {
        Sid    = "ReplanTokenRead"
        Effect = "Allow"
        ...
        Resource = [
          "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter${var.replan_token_param}",
        ]
      },
      {
        Sid      = "Logs"
...
resource "aws_lambda_function" "replan_agenda" {
  function_name    = "replan-agenda"
  ...
  environment {
    variables = {
      QUEUE_URL   = data.aws_sqs_queue.user_commands.url
      TOKEN_PARAM = var.replan_token_param
    }
  }
...
# Public HTTPS entry point for replan triggers. Auth NONE by design: request
# authenticity is enforced inside the handler (bearer token in X-HyperBrain-Token
# header checked via constant-time compare against the SSM secret).
resource "aws_lambda_function_url" "replan_agenda" {
  function_name      = aws_lambda_function.replan_agenda.function_name
